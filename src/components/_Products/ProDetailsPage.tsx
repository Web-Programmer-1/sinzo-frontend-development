...
function ReviewsSection({
  ...
  const { data: reviewsRes, isLoading } = useGetReviewsByProduct(productId as any);
  type CompatibleReviewType = TReview | Review;
  const reviews: CompatibleReviewType[] = (reviewsRes?.data as CompatibleReviewType[]) || [];
  const { mutate: createReview, isPending: isCreating } = useCreateReview(productId as any);
...
const { mutate: addReply, isPending: isReplying } = useAddReplyToReview({
  productId,
  params: { sort: sort as "latest" | "oldest" },
});
...
{showReplies && review.replies.map((rep:any) => (
  <div key={rep.replyId} className="rv-reply-item">
    <div
      className="rv-av rv-av--sm relative overflow-hidden"
      style={{
        background:
          AV_COLORS[(rep.userName?.charCodeAt(0) || 0) % AV_COLORS.length],
      }}
    >
      {rep.userImage ? (
        <Image
        ...
        <span>{(rep.userName || "U").charAt(0).toUpperCase()}</span>
      )}
    </div>

    <div className="rv-reply-body">
      <div className="rv-meta-row">
        <span className="rv-name">{rep.userName || "User"}</span>
