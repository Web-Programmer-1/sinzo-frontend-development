...
import { useGetOrderById } from "../../Apis/order";
...
const StatCard = ({
  ...
  return (
    <div className="rounded-3xl border border-black/10 bg-white p-5 shadow-sm">
      ...
      {subValue ? <p className="mt-2 text-sm text-gray-500">{subValue}</p> : null}
    </div>
  );
};
...
const InfoRow = ({
  ...
  return (
    <div className="flex items-start justify-between gap-4 border-b border-black/5 py-3 last:border-b-0">
      ...
      <div className="text-right text-sm font-semibold text-black break-words">
        {value || "N/A"}
      </div>
    </div>
  );
  ...
  return (
    <section className="min-h-screen bg-[#fafafa] px-4 py-8 md:px-6 lg:px-8">
      <div className="mx-auto max-w-7xl space-y-6">
        <div className="animate-pulse rounded-3xl border border-black/10 bg-white p-6 shadow-sm">
        ...
        <div className="grid grid-cols-2 gap-4 xl:grid-cols-4">
          {Array.from({ length: 4 }).map((_, i) => (
            <div
              key={i}
              className="h-32 animate-pulse rounded-3xl border border-black/10 bg-white shadow-sm"
            />
          ))}
        </div>

        <div className="grid grid-cols-1 gap-6 xl:grid-cols-3">
          <div className="xl:col-span-2 h-[420px] animate-pulse rounded-3xl border border-black/10 bg-white shadow-sm" />
          <div className="h-[420px] animate-pulse rounded-3xl border border-black/10 bg-white shadow-sm" />
        </div>
      </div>
    </section>
  );
    ...
    <section className="min-h-screen bg-[#fafafa] px-4 py-10 md:px-6 lg:px-8">
      <div className="mx-auto max-w-4xl">
        <div className="rounded-3xl border border-red-200 bg-red-50 px-6 py-14 text-center shadow-sm">
          <h2 className="text-2xl font-bold text-red-700">Failed to load order</h2>
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mb-8 rounded-[28px] border border-black/10 bg-white p-5 shadow-sm md:p-7">
          <div className="flex flex-col gap-5 xl:flex-row xl:items-center xl:justify-between">
            <div>
              ...
              <p className="mt-2 text-sm leading-6 text-gray-500">
                Full order overview including customer, payment, courier,
                product items, and order timeline.
              </p>
            </div>

            <div className="flex flex-wrap gap-3">
              <span
                className={`inline-flex rounded-full border px-4 py-2 text-sm font-semibold ${statusClasses[order.orderStatus] || "border-gray-200 bg-gray-50 text-gray-700"}`}
              >
                {formatStatus(order.orderStatus)}
              </span>

              <span
                className={`inline-flex rounded-full border px-4 py-2 text-sm font-semibold ${paymentClasses[order.paymentStatus] || "border-gray-200 bg-gray-50 text-gray-700"}`}
              >
              ...
              {order.courierStatus ? (
                <span
                  className={`inline-flex rounded-full border px-4 py-2 text-sm font-semibold ${courierClasses[order.courierStatus] || "border-gray-200 bg-gray-50 text-gray-700"}`}
                >
          ...
          <div className="mt-6 flex flex-wrap gap-3">
            ...
            <Link
              ...
              className="inline-flex items-center justify-center rounded-2xl bg-gradient-to-r from-violet-500 via-fuchsia-500 to-purple-600 px-4 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:scale-[1.01] hover:shadow-md"
            >
              Edit Order
            </Link>
            ...
            <Link
              ...
              className="inline-flex items-center justify-center rounded-2xl bg-gradient-to-r from-rose-500 via-red-500 to-orange-500 px-4 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:scale-[1.01] hover:shadow-md"
            >
              Delete Order
            </Link>
          </div>
        ...
        <div className="mb-8 grid grid-cols-2 gap-4 xl:grid-cols-4">
          <StatCard
            ...
            subValue={`Subtotal ${formatCurrency(order.subtotal)}`}
          />
          <StatCard
            title="Due Amount"
            value={formatCurrency(order.dueAmount)}
            subValue={`Paid ${formatCurrency(order.paidAmount)}`}
          />
          <StatCard
            title="Total Items"
            value={order.items?.length || 0}
            subValue={`Quantity ${totalQty}`}
          />
          <StatCard
            title="Placed On"
            value={formatDate(order.createdAt)}
            subValue={`Serial #${order.serialNumber}`}
          />
        </div>
          ...
          <div className="space-y-6 xl:col-span-2">
            ...
            <div className="rounded-[28px] border border-black/10 bg-white p-5 shadow-sm md:p-6">
              <div className="mb-5 flex items-center justify-between gap-3">
                ...
                <div className="rounded-full bg-gray-100 px-4 py-2 text-sm font-semibold text-black">
              ...
              <div className="space-y-4">
                {order.items?.map((item) => (
                  <div
                    key={item.id}
                    className="rounded-3xl border border-black/10 bg-[#fcfcfc] p-4"
                  >
                        ...
                        {item.productSlug ? (
                          <div className="mt-4">
                            <Link
                              ...
                              className="inline-flex items-center rounded-xl border border-black/10 bg-white px-3 py-2 text-sm font-semibold text-black transition hover:border-black"
                            >
                              View Product
                            </Link>
                          </div>
              ...
              <div className="rounded-[28px] border border-black/10 bg-white p-5 shadow-sm md:p-6">
                ...
                <div className="mt-5">
                  ...
                  <InfoRow
                    ...
                    value={
                      <span
                        className={`inline-flex rounded-full border px-3 py-1 text-xs font-semibold ${paymentClasses[order.paymentStatus] || "border-gray-200 bg-gray-50 text-gray-700"}`}
                      >
                        {formatStatus(order.paymentStatus)}
                      </span>
                    }
                  ...
                  <InfoRow
                    ...
                    value={
                      <span
                        className={`inline-flex rounded-full border px-3 py-1 text-xs font-semibold ${statusClasses[order.orderStatus] || "border-gray-200 bg-gray-50 text-gray-700"}`}
                      >
                        {formatStatus(order.orderStatus)}
                      </span>
                    }
          ...
          <div className="space-y-6">
            ...
            <div className="rounded-[28px] border border-black/10 bg-white p-5 shadow-sm md:p-6">
              ...
              <div className="mt-5">
                ...
                <InfoRow
                  label="Account"
                  value={order.user ? "Registered User" : "Guest Order"}
                />
            ...
            <div className="rounded-[28px] border border-black/10 bg-white p-5 shadow-sm md:p-6">
              ...
              <div className="mt-5">
                ...
                <InfoRow
                  label="Delivery Area"
                  value={formatStatus(order.deliveryArea)}
                />
                <InfoRow
                  label="Address"
                  value={order.addressLine || "N/A"}
                />
              ...
              <div className="mt-5">
                ...
                <InfoRow
                  ...
                  value={
                    order.courierStatus ? (
                      <span
                        className={`inline-flex rounded-full border px-3 py-1 text-xs font-semibold ${courierClasses[order.courierStatus] || "border-gray-200 bg-gray-50 text-gray-700"}`}
                      >
                        {formatStatus(order.courierStatus)}
                      </span>
                    ) : (
                ...
                <InfoRow
                  ...
                  value={formatDateTime(order.courierSentAt || undefined)}
                />
                <InfoRow
                  label="Courier Note"
                  ...
                  order.statusHistory.map((history, index) => (
                    <div key={history.id} className="flex gap-3">
                      <div className="flex flex-col items-center">
                        <div className="mt-1 h-3.5 w-3.5 rounded-full bg-black" />
                      ...
                      <div className="min-w-0 flex-1 rounded-2xl bg-gray-50 p-4">
                        <div className="flex flex-wrap items-center gap-2">
                          <span
                            className={`inline-flex rounded-full border px-3 py-1 text-xs font-semibold ${statusClasses[history.status] || "border-gray-200 bg-gray-50 text-gray-700"}`}
                          >
