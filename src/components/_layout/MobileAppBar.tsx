...
const tabs: { id: TabId; label: string; icon: (active: boolean) => any }[] = [
  {
    ...
    label: "Home",
    icon: (active) => (
      <svg width="22" height="22" viewBox="0 0 24 24" fill={active ? "#111" : "none"} stroke="#111" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
        <path d="M3 9.5L12 3l9 6.5V20a1 1 0 0 1-1 1H4a1 1 0 0 1-1-1V9.5z"/>
        <path d="M9 21V12h6v9" fill={active ? "#fff" : "none"} stroke="#111" strokeWidth="1.8"/>
      </svg>
    ),
  },
  {
  ...
  },
  {
    ...
    icon: (active) => (
      <svg width="22" height="22" viewBox="0 0 24 24" fill={active ? "#111" : "none"} stroke="#111" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
        <circle cx="9" cy="21" r="1" fill="#111"/><circle cx="20" cy="21" r="1" fill="#111"/>
...
export default function MobileNav({ cartCount = 1, activeTab = "home", onTabChange }: MobileNavProps) {
  ...
  return (
    <>
      ...
      <nav className="sinzo-mobile-nav" role="navigation" aria-label="Mobile navigation">
        {tabs.map((tab) => {
          ...
          return (
            ...
            >
              {/* Ripple effect */}
              {ripple?.id === tab.id && (
                <span key={ripple.key} className="sinzo-ripple" aria-hidden="true" />
              )}
              ...
              <span className="sinzo-tab-icon">
                ...
                {tab.id === "cart" && cartCount > 0 && (
                  <span className="sinzo-cart-badge" aria-label={`${cartCount} items in cart`}>
